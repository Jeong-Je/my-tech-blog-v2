...
public:
    // 순환 참조의 문제를 파악하기 일부러 "다른 저금통의 주소"를 가질 수 있게 합니다.
    std::shared_ptr<PiggyBank> bestFriend;

    PiggyBank(std::string name, int initCoin) : name(name), amount(initCoin) {
        std::cout << "[생성] " << name << " 저금통 탄생!\n";
    }

    ~PiggyBank() {
        std::cout << "[소멸] " << name << " 저금통이 사라집니다! (청소 완료)\n";
    }
};

int main() {

    auto myPiggy = std::make_shared<PiggyBank>("상훈이", 1111); // myPiggy 카운트=1
    auto yrPiggy = std::make_shared<PiggyBank>("예림이", 2222); // yrPiggy 카운트=1

    myPiggy->bestFriend = yrPiggy; // yrPiggy의 카운트=2
    yrPiggy->bestFriend = myPiggy; // myPiggy의 카운트=2

    std::cout << "현재 myPiggy의 주인 수: " << myPiggy.use_count() << "명\n";
    std::cout << "현재 yrPiggy의 주인 수: " << yrPiggy.use_count() << "명\n";
...
yrPiggy->bestFriend = myPiggy; // myPiggy의 카운트=2
```
위 코드의 순환 참조 문제에 의해서 소멸자가 호출되지 않는다. 이를 해결하기 위해서
**PiggyBank** 클래스의 **bestFriend**의 타입을 아래처럼 `weak_ptr`로 수정하면 해결된다.
```cpp
std::weak_ptr<PiggyBank> bestFriend;
```
***